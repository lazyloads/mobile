...
gulp.task('jade:main', function() {
  var combined = combiner.obj([
    gulp.src([dir.srcJade + '*.jade']),
    changed(dir.srcJade, { extension: '.html' }),
    jade(),
    rename({ suffix: '' }),
    gulp.dest(dir.dest),
    livereload(),
    tap(function(file) {
      handleComplete('jade', file);
    })
  ]);
...
gulp.task('default', function() {
  ...
  gulp.watch([dir.srcJade + '*.jade'], ['jade:main', 'refresh']);
...
// 全部重新发布任务
gulp.task('publish', ['jade:main', 'sass:main', 'uglify:main', 'uglify:part', 'staticHash', 'refresh']);
